...
#ifdef CONFIG_PM_WARP
		if (pm_device_down) {
			bank->pm_regs.swporta_dr =
				__raw_readl(bank->reg_base + GPIO_SWPORT_DR);
			bank->pm_regs.swporta_ddr =
				__raw_readl(bank->reg_base + GPIO_SWPORT_DDR);
			bank->pm_regs.inttype_level =
				__raw_readl(bank->reg_base +
					    GPIO_INTTYPE_LEVEL);
			bank->pm_regs.polarity =
				__raw_readl(bank->reg_base + GPIO_INT_POLARITY);
		}
#endif
...
#ifdef CONFIG_PM_WARP
		if (pm_device_down) {
			__raw_writel(bank->pm_regs.swporta_ddr,
				     bank->reg_base + GPIO_SWPORT_DDR);
			__raw_writel(bank->pm_regs.inttype_level,
				     bank->reg_base + GPIO_INTTYPE_LEVEL);
			__raw_writel(bank->pm_regs.polarity,
				     bank->reg_base + GPIO_INT_POLARITY);
			__raw_writel(bank->pm_regs.swporta_dr,
				     bank->reg_base + GPIO_SWPORT_DR);
		}
#endif
