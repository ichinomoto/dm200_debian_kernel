...
static int ohci_rk_pm_suspend(struct device *dev)
{
	struct usb_hcd *hcd = dev_get_drvdata(dev);
...
static int ohci_rk_pm_resume(struct device *dev)
{
	struct usb_hcd *hcd = dev_get_drvdata(dev);
...
SIMPLE_DEV_PM_OPS( ohci_rk_dev_pm_ops, ohci_rk_pm_suspend, ohci_rk_pm_resume);
#endif
