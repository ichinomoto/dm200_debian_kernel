...
static ssize_t 
MaxPower_configureation_store(struct device *pdev, struct device_attribute *attr,
					const char *buf, size_t size)
{
	...
	ret = kstrtol(buf, 10, &MaxPower);
	if(ret != 0){
		mutex_unlock(&dev->mutex);
		return -EINVAL;
	}
	android_config_driver.MaxPower = MaxPower;

	mutex_unlock(&dev->mutex);
	return size;
}
static ssize_t
MaxPower_configureation_show(struct device *pdev,
...
static DEVICE_ATTR(MaxPower, S_IRUGO | S_IWUSR, MaxPower_configureation_show,
					       MaxPower_configureation_store);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%s", config->common->inquiry_string) != 1)
		return -EINVAL;
#else
	strncpy(config->common->inquiry_string, buf, size+1);
#endif
	return size;
}
