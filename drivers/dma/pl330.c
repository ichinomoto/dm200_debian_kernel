...
static u32 pl330_inten;
static int pl330_suspend (struct amba_device *adev, pm_message_t state)
{
	if (pm_device_down) {
		struct dma_pl330_dmac *pdmac = amba_get_drvdata(adev);
		struct pl330_info *pi;
		...
		pi = &pdmac->pif;
		regs = pi->base;
		pl330_inten = readl(regs + INTEN);
	}
	return 0;
}

static int pl330_resume (struct amba_device *adev)
{
	if (pm_device_down) {
		struct dma_pl330_dmac *pdmac = amba_get_drvdata(adev);
		struct pl330_info *pi;
