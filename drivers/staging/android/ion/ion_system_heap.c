...
static int ion_system_heap_allocate_rk(struct ion_heap *heap,
				     struct ion_buffer *buffer,
				     unsigned long size, unsigned long align,
				     unsigned long flags)
{
	int ret;
	if(flags & ION_FLAG_STRAIGH_BUF)
		ret = ion_system_contig_heap_allocate(heap, buffer, size, align, flags);
	else
		ret = ion_system_heap_allocate(heap, buffer, size, align, flags);
	return ret;
}
...
static void ion_system_heap_free_rk(struct ion_buffer *buffer)
{
	if(buffer->flags & ION_FLAG_STRAIGH_BUF)
...
static struct ion_heap_ops system_heap_ops = {
	.allocate = ion_system_heap_allocate_rk,
	.free = ion_system_heap_free_rk,
