...
#if defined(CONFIG_AP6210) || defined(CONFIG_AP6335) || defined(CONFIG_AP6212) || defined(CONFIG_AP6236)
...
static int rfkill_rk_pm_suspend(struct device *dev)
{
	struct rfkill_rk_data *rfkill = g_rfkill;
	struct rfkill_rk_irq* irq = &(rfkill->pdata->wake_host_irq);
...
static int rfkill_rk_pm_resume(struct device *dev)
{
	struct rfkill_rk_data *rfkill = g_rfkill;
	struct rfkill_rk_irq* irq = &(rfkill->pdata->wake_host_irq);
	int ret = 0;
	...
	if (gpio_is_valid(rfkill->pdata->poweron_gpio.io))
		gpio_direction_output(rfkill->pdata->poweron_gpio.io, !rfkill->pdata->poweron_gpio.enable);

	if (gpio_is_valid(rfkill->pdata->reset_gpio.io))
		gpio_direction_output(rfkill->pdata->reset_gpio.io, !rfkill->pdata->reset_gpio.enable);
...
#ifdef CONFIG_PM_WARP
	.freeze = rfkill_rk_pm_suspend,
	.thaw = rfkill_rk_pm_resume,
	.restore = rfkill_rk_pm_resume,
#endif
};
